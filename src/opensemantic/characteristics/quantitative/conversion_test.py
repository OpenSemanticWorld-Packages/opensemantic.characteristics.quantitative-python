...
from opensemantic.characteristics.quantitative._static import quantity_registry, unit_registry
import json
...
def test_quantity_value():
    ...
    assert q_json == {
        ...
        "value": 1.0,
        "unit": "Item:OSWf101d25e944856e3bd4b4c9863db7de2#OSW322dec469be75aedb008b3ebff29db86"
    }
        ...
        for ue in qu_reg[qv]:
            for u in ue:
                ...
                try:
                    ...
                    #assert q1 == q_
                    if q1 != q_:
                        #print(f"Warning: {q1.value} {q1.unit} != {q_.value} {q_.unit}")
                        warning_count += 1
                        #print(q1.to_pint())
                        #print(q_.to_pint())
                        if type(q1) != type(q_):
                            print(f"Critical Warning: {q1.__class__.__name__} and {q_.__class__.__name__} have the same unit {u}")
                    ...
                    if isinstance(e, pint.errors.UndefinedUnitError):
                        ...
                        #print(re
                    error_count += 1
    print(f"{success_count} successful, {error_count} errors and {warning_count} warnings ({critical_warning_count} critical)")
