...
from oold.model import LinkedBaseModelMetaClass as ModelMetaclass
...
from opensemantic.core import Characteristic
...
class QuantityValueMetaclass(ModelMetaclass):
    def __new__(cls, clsname, bases, attrs, **kwargs):
        class_instance = super().__new__(cls, clsname, bases, attrs, **kwargs)
        # Only register classes that define their OWN unit field (not inherited).
        # Check attrs (class namespace) like v1 does, but use model_fields for
        # resolved type extraction (avoids string annotation issues).
        if "unit" in attrs and attrs["unit"] is not None:
            if (
                hasattr(class_instance, "model_fields")
                and "unit" in class_instance.model_fields
            ):
                unit_field = class_instance.model_fields["unit"]
                if unit_field.default is not None:
                    unit_field_type = _unwrap_optional(unit_field.annotation)
                    if unit_field_type is not None and unit_field_type is not type(
                        None
                    ):
...
class QuantityValue(OswBaseModel, metaclass=QuantityValueMetaclass):
    ...
    def accept_any_unit_enum(cls, v):
        ...
        expected_enum = _unwrap_optional(cls.model_fields["unit"].annotation)
    ...
    def from_pint(
        ...
        def original(quantity_: pint.Quantity):
            ...
            if return_dict:
                return {
                    ...
                    "type": quantity_class.model_fields["type"].default,
                }
        ...
        def by_dimensionality(quantity_: pint.Quantity):
            """Fallback when the exact unit symbol is not in unit_registry.

            Resolves the quantity by its dimensionality: converts it to a
            representative SI-coherent unit of the same dimensionality
            (DIMENSION_TO_UNIT, generated by scripts/post_process.py) and builds
            the QuantityValue from that. Lets derived units (watt, joule, ohm,
            ...) and composed results (e.g. volt*ampere) round-trip.
            """
            try:
                from opensemantic.characteristics.quantitative._dimensions import (
                    DIMENSION_TO_UNIT,
                )
            except ImportError:
                DIMENSION_TO_UNIT = {}
            dim = str(quantity_.dimensionality) or "dimensionless"
            member = DIMENSION_TO_UNIT.get(dim)
            if member is None or member not in unit_registry:
                raise KeyError(f"No registered unit for dimensionality '{dim}'")
            unit_class = unit_registry[member][0]
            pint_name = QuantityValue.get_pint_ureg_compatible_str(member)
            converted = quantity_.to(pint_name)
            quantity_class = class_logic(unit_class)
            if return_dict:
                return {
            ...
            return quantity_class(
                value=converted.magnitude, unit=unit_class[member]
            )
    ...
    def __eq__(self, other: "QuantityValue") -> bool:
        # Guard against comparison with non-QuantityValue operands (e.g. None).
        # pydantic's exclude_defaults compares each field value to its default
        # (often None), which would otherwise call None.to_pint().
...
def _get_row_class(cls):
    """Extract the row type from a model's 'rows' field (e.g. List[T] -> T)."""
    if "rows" not in cls.model_fields:
        return OswBaseModel
    annotation = cls.model_fields["rows"].annotation
    args = typing.get_args(annotation)
    return args[0] if args else annotation


def _get_unit_default_name(row_class, attr):
    """Get the default unit name for a field's type (e.g. Length -> meter)."""
    attr_type = _unwrap_optional(row_class.model_fields[attr].annotation)
    return attr_type.model_fields["unit"].default.name
...
class TabularData(OswBaseModel):
    ...
    def to_df(self) -> pd.DataFrame:
        series = []
        row_class = _get_row_class(self.__class__)
        for attr in row_class.model_fields.keys():
            if attr == "type":
                continue
            q_name = QuantityValue.get_pint_ureg_compatible_str(
                _get_unit_default_name(row_class, attr)
            )
            s = pd.Series(
                [
                    (
    ...
    def from_df(cls, df: pd.DataFrame):
        rows = []
        row_class = _get_row_class(cls)

        additional_fields = {}
        for key in df.columns:
            if key not in row_class.model_fields.keys():
